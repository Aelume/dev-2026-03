...
def export_results(df, stats, output_dir="output"):
    """将清洗后的数据和统计信息导出到 output/ 目录。"""
    os.makedirs(output_dir, exist_ok=True)

    # 导出清洗后的 CSV
    cleaned_csv_path = os.path.join(output_dir, "cleaned_retail.csv")
    df.to_csv(cleaned_csv_path, index=False)
    print(f"清洗后数据已导出到：{cleaned_csv_path}")

    # 导出统计信息到 summary.json
    summary_json_path = os.path.join(output_dir, "summary.json")
    with open(summary_json_path, "w", encoding="utf-8") as f:
        json.dump(stats, f, ensure_ascii=False, indent=2)
    print(f"统计信息已导出到：{summary_json_path}")
    ...
    with open(summary_txt_path, "w", encoding="utf-8") as f:
        ...
        for key, value in stats.items():
            f.write(f"{key}：{value}\n")
    print(f"统计信息已导出到：{summary_txt_path}")
